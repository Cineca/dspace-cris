...
public class ResearcherImageServlet extends HttpServlet {
	/** log4j category */
	private static Logger log = Logger.getLogger(ResearcherImageServlet.class);
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse response)
		...
		Integer id = ResearcherPageUtils
		...
		try {
			...
			TableRow row = DatabaseManager.find(context,
					"model_researcher_page", id);
			ext = row.getStringColumn("pict_ext");
			contentType = row.getStringColumn("pict_type");
			if (ext != null && !ext.isEmpty() && contentType != null
				...
				File image = new File(
						ConfigurationManager
								...
								+ File.separatorChar
								+ authorityKey
								+ File.separatorChar + authorityKey + "." + ext);
