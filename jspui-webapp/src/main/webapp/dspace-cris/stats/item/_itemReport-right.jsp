...
<div class="stats-tab<c:if test="${type ne 'bitstream'}"> stats-tab-current </c:if>"><a href="${link}"><fmt:message key="view.stats-item.selectedObject.page.title" /></a></div>
<div class="stats-tab<c:if test="${type eq 'bitstream'}"> stats-tab-current </c:if>"><a href="${link}&amp;type=bitstream"><fmt:message key="view.stats-item.top.bitstream.page.title" /></a></div>
