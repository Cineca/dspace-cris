...
<c:set var="dspace.layout.head" scope="request">
	...
	j(document).ready(function() {
		j("#create").click(function() {
			location.href = '<%= request.getContextPath() %>/rp/administrator/webservices/edit.htm';			
		});		
	});
    </script>
</c:set>
<dspace:layout locbar="link" navbar="admin"	titlekey="jsp.dspace-admin.rp">
	...
	<display:table name="${listUsers}" cellspacing="0" cellpadding="0" 
			...
			<c:when test="${!empty objectList.username}">
				...
				<display:column titleKey="jsp.layout.table.hku.ws.info" url="/rp/administrator/webservices/edit.htm" paramId="id" paramProperty="id" sortable="true">
					${objectList.username}				
				</display:column>
			</c:when>
			<c:otherwise>			
			...
			<display:column>
				<a href="<%= request.getContextPath() %>/rp/administrator/webservices/delete.htm?id=${objectList.id}"><fmt:message key="jsp.layout.hku.ws.delete"/></a>		
			</display:column>
