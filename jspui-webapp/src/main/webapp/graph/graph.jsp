...
<style type="text/css">
.ui-layout-pane-west { background: url(../images/background.jpg) left top repeat-x #BDBDBD; border: 0px; overflow-x: hidden;}
...
.ui-layout-resizer-west { background: url(../images/background.jpg) left top repeat-x #BDBDBD; border-width: 0px; }
...
#slider-zoom-out { position: relative; background: #ccc; padding: 0px 5px 0px 4px; font-weight: bold; cursor: pointer; z-index: 5; margin-top: -3px; }
.ui-slider-horizontal { height: .4em; }
.ui-slider-horizontal .ui-slider-handle { top: -.5em; width: .3em; }
.ui-slider-vertical { width: .1em; }
...
<body>
	...
	<div id="main-rp">
						...
						<c:choose>
							<c:when
								test="${!empty researcher.pict && !empty researcher.pict.value && researcher.pict.visibility==1}">
										...
										<a target="_blank" href="../rp/${authority}">
					...
					<div class="rp-content">
							<div class="rp-name"><a target="_blank" href="../rp/${authority}"><c:if
						...
						<ul><li><c:url var="deptSearch"
							...
							<a target="_blank" href="<%=request.getContextPath()%>/dnetwork/graph?dept=<%=URLEncoder.encode(researcher.getDept().getValue(),"UTF-8")%>"><span class="icon-network"><img src='../image/wheel-icon2.jpg' alt="<fmt:message key='jsp.network.label.link.network.dept'/>" title="<fmt:message key='jsp.network.label.link.network.dept'/>"/></span></a>
		...
		<div id="loadingprofile" style="display: none">
			<img src="<%=request.getContextPath()%>/image/bar-loader.gif" id="loadertarget" />
		</div>
