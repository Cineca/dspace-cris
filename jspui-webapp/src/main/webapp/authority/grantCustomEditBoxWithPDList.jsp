...
<dspace:layout locbar="link" navbar="admin"
	...
	<form:form commandName="box" method="post">
		...
		<c:set var="sortedAllContainablesList" value="${researcher:sortBoxByComparator(allContainablesList,'it.cilea.hku.authority.webui.comparator.CustomContainableComparator')}" />
			...
			<c:if test="${!empty box.id}">			
				...
				key="jsp.dspace-admin.hku.jdyna-configuration.editdynamicfield" />"
					...
					<spring:bind path="mask">
						...
						<input class="jdynacontainable" type="hidden" value="${optionToCompare}" id="mask_${boxed.id}" name="mask" />
					</spring:bind>
					</div>
					</c:if>
				</c:if>
		...
		<c:if test="${!empty box.id}">
			<div style="padding: 0; margin: 0 10px;"><a
				href="<%=request.getContextPath()%>/${specificPartPath}/administrator/createDateDynamicField.htm?boxId=${box.id}&tabId=${tabId}">
			<fmt:message
				key="jsp.dspace-admin.hku.jdyna-configuration.newdatedynamicfield" />
			</a></div>
			<div style="padding: 0; margin: 0 10px;"><a
				href="<%=request.getContextPath()%>/${specificPartPath}/administrator/createTextDynamicField.htm?boxId=${box.id}&tabId=${tabId}">
			<fmt:message
				key="jsp.dspace-admin.hku.jdyna-configuration.newtextdynamicfield" />
			</a></div>
			<div style="padding: 0; margin: 0 10px;"><a
				href="<%=request.getContextPath()%>/${specificPartPath}/administrator/createNestedDynamicField.htm?boxId=${box.id}&tabId=${tabId}">
			<fmt:message
				key="jsp.dspace-admin.hku.jdyna-configuration.newnesteddynamicfield" />
			</a></div>
			<div style="padding: 0; margin: 0 10px;"><a
				href="<%=request.getContextPath()%>/${specificPartPath}/administrator/createLinkDynamicField.htm?boxId=${box.id}&tabId=${tabId}">
			<fmt:message
				key="jsp.dspace-admin.hku.jdyna-configuration.newlinkdynamicfield" />
			</a></div>
			<div style="padding: 0; margin: 0 10px;"><a
				href="<%=request.getContextPath()%>/${specificPartPath}/administrator/createFileDynamicField.htm?boxId=${box.id}&tabId=${tabId}">
			<fmt:message
