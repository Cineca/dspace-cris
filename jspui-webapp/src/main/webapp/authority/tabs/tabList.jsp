...
<dspace:layout locbar="link" navbar="admin"
		...
		<c:forEach items="${listTab}" var="tab">
			...
			<div style="padding: 0; margin: 0 10px;">${tab.title} <c:if
				...
				key="jsp.dspace-admin.hku.jdyna-configuration.deletedynamicfield" />"
					href="<%=request.getContextPath()%>/${specificPartPath}/administrator/deleteTabs.htm?id=${tab.id}">
				<img
				...
				href="<%=request.getContextPath()%>/${specificPartPath}/administrator/editTabs.htm?id=${tab.id}">
			<img
	...
	<div style="padding: 0; margin: 0 10px;"><a
		href="<%=request.getContextPath()%>/${specificPartPath}/administrator/createTabs.htm">
